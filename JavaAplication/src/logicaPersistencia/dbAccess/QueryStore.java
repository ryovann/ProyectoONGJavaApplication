...
public class QueryStore {
	public String select_NuevosUsuarios_Lista__CIVenezolana(){
		...
		String query = "SELECT * FROM PERSONA P INNER JOIN DOCUMENTOS D ON D.ID_PERSONA = P.ID_PERSONA WHERE P.ESTADO = 0 AND D.CI_VENEZOLANA = (?)";
		return query;
	}
	...
	public String select_NuevosUsuarios_Lista__NombreApellido(){
		...
		String query = "SELECT * FROM PERSONA P INNER JOIN DOCUMENTOS D ON D.ID_PERSONA = P.ID_PERSONA WHERE P.ESTADO = 0 AND CONCAT(PRIMER_NOMBRE,' ',PRIMER_APELLIDO) LIKE (?)";
		return query;
	}
